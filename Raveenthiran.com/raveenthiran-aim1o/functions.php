<?php
/**
 * aim1o — the master theme. A magical, heavily-3D experience built on
...
if ( ! function_exists( 'mk_scroll_mode' ) ) {
	/**
	 * aim1o is a single, continuous HORIZONTAL slider: every page rides the
...
add_action( 'wp_enqueue_scripts', function () {
	...
	// aim1o master layer — identity + scroll-mode engine (no-scroll /
	// horizontal / vertical per page) + the Magic Control wiring.
	wp_enqueue_style( 'mk-skin', get_template_directory_uri() . '/assets/css/moksha.css', [ 'void-skin' ], NR_THEME_VERSION );
	wp_enqueue_script( 'mk-scroll', get_template_directory_uri() . '/assets/js/mk-scroll.js', [], NR_THEME_VERSION, true );
	wp_localize_script( 'mk-scroll', 'MK', [
		'mode' => function_exists( 'mk_scroll_mode' ) ? mk_scroll_mode() : 'v',
		'fx'   => [
			'webgl'    => nr_opt( 'nr_fx_webgl', '1' ) === '1',
			'orb'      => nr_opt( 'mk_fx_orb', '1' ) === '1',
			'trails'   => nr_opt( 'mk_fx_trails', '1' ) === '1',
			'tilt'     => nr_opt( 'mk_fx_tilt', '1' ) === '1',
			'hint'     => nr_opt( 'mk_fx_hint', '1' ) === '1',
			'intensity'=> (int) nr_opt( 'mk_fx_intensity', 100 ),
		],
	] );
