...
function nr_settings_defaults() {
	return [
		...
		/* aim1o — Magic Control (steer the 3D / motion layer) */
		'mk_fx_orb'          => '1',   // singularity orb on the home
		'mk_fx_trails'       => '1',   // cursor light trails
		'mk_fx_tilt'         => '1',   // 3D hover tilt on frames
		'mk_fx_hint'         => '1',   // "scroll to travel" hint on horizontal pages
		'mk_fx_intensity'    => '100', // global motion intensity 0–200 (%)
...
add_action( 'admin_menu', function () {
	add_submenu_page(
		'nr-control',
		__( 'aim1o — Theme Settings', 'raveenthiran' ),
...
function nr_theme_settings_page() {
	...
	<div class="wrap nr-settings">
		<header class="nr-adm-head">
			<span class="nr-adm-eyebrow"><span class="nr-adm-rule"></span>THEME CONTROL // aim·1o</span>
			<h1 class="nr-adm-title">aim1o <span class="nr-adm-gold">Settings</span></h1>
		...
		<form method="post" action="options.php" class="nr-settings-form">
			...
			<details open class="nr-settings__group">
				...
				<table class="form-table" role="presentation">
					...
					<tr><th><label><?php esc_html_e( 'Home hero image (URL)', 'raveenthiran' ); ?></label></th>
						<td><?php nr_field_text( 'nr_home_hero', 60 ); ?>
							<p class="description"><?php esc_html_e( 'Full-bleed AI hero on the home page. Upload your image to Media, paste its URL here. Blank = a designed obsidian + gilt-light hero. (Or drop a file at the theme’s assets/img/aim1o-hero.jpg.)', 'raveenthiran' ); ?></p></td></tr>
			...
			<details open class="nr-settings__group">
				<summary><h2>✦ Magic Control (aim1o — steer the 3D)</h2></summary>
				<table class="form-table" role="presentation">
					<tr><td colspan="2"><p class="description"><?php esc_html_e( 'The master switches for the magical layer. The WebGL 3D hero itself is under Effects → “WebGL 3D”. Everything degrades gracefully on mobile / reduced-motion.', 'raveenthiran' ); ?></p></td></tr>
					<tr><th><label><?php esc_html_e( 'Singularity orb (home)', 'raveenthiran' ); ?></label></th>
						<td><?php nr_field_toggle( 'mk_fx_orb', __( 'Show the orbiting core behind the home headline', 'raveenthiran' ) ); ?></td></tr>
					<tr><th><label><?php esc_html_e( 'Cursor light trails', 'raveenthiran' ); ?></label></th>
						<td><?php nr_field_toggle( 'mk_fx_trails', __( 'Gilt light follows the pointer (desktop)', 'raveenthiran' ) ); ?></td></tr>
					<tr><th><label><?php esc_html_e( '3D hover tilt', 'raveenthiran' ); ?></label></th>
						<td><?php nr_field_toggle( 'mk_fx_tilt', __( 'Frames lean toward the pointer in 3D', 'raveenthiran' ) ); ?></td></tr>
					<tr><th><label><?php esc_html_e( 'Horizontal scroll hint', 'raveenthiran' ); ?></label></th>
						<td><?php nr_field_toggle( 'mk_fx_hint', __( 'Show the “scroll to travel” cue on horizontal pages', 'raveenthiran' ) ); ?></td></tr>
					<tr><th><label><?php esc_html_e( 'Motion intensity (0–200%)', 'raveenthiran' ); ?></label></th>
						<td><input type="range" min="0" max="200" step="10" name="mk_fx_intensity" value="<?php echo esc_attr( nr_opt( 'mk_fx_intensity', 100 ) ); ?>" oninput="this.nextElementSibling.textContent=this.value+'%'"><span style="margin-left:10px;font-family:monospace"><?php echo esc_html( nr_opt( 'mk_fx_intensity', 100 ) ); ?>%</span>
							<p class="description"><?php esc_html_e( 'Global multiplier for the motion layer — 0 = still, 100 = default, 200 = maximum drama. Drives displacement, parallax and tilt strength.', 'raveenthiran' ); ?></p></td></tr>
				</table>
			</details>
