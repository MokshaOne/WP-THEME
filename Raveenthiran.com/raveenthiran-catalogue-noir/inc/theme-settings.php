...
function nr_settings_defaults() {
	return [
		...
		'nr_latepoint_shortcode' => '', /* paste the exact LatePoint shortcode that renders inline for your install */
		...
		<form method="post" action="options.php" class="nr-settings-form">
				...
				<table class="form-table" role="presentation">
					...
					<tr><th><label><?php esc_html_e( 'Booking URL', 'raveenthiran' ); ?></label></th>
						<td><?php nr_field_text( 'nr_booking_url', 60 ); ?>
							<p class="description"><?php esc_html_e( 'Optional. If set, the booking modal embeds this URL in an iframe. Leave blank to use the inline fallback form (or LatePoint if installed).', 'raveenthiran' ); ?></p></td></tr>
				</table>
			</details>
			...
			<details open class="nr-settings__group">
				...
				<table class="form-table" role="presentation">
					...
					<tr><th><label><?php esc_html_e( 'LatePoint shortcode', 'raveenthiran' ); ?></label></th>
						<td>
							<?php nr_field_text( 'nr_latepoint_shortcode', 80 ); ?>
							<p class="description">
								<?php esc_html_e( 'Paste the exact LatePoint shortcode you want to render inline in the booking modal and on the booking page. Leave empty to auto-detect. Common options to try:', 'raveenthiran' ); ?>
							</p>
							<ul style="margin:6px 0 0 18px;color:#555;font-size:12px;list-style:disc">
								<li><code>[latepoint_book_button]</code> — <?php esc_html_e( 'standard, opens calendar in an overlay when the button is clicked', 'raveenthiran' ); ?></li>
								<li><code>[latepoint_book_button caption="Buchen" trigger="text"]</code> — <?php esc_html_e( 'styled as a text link instead of a button', 'raveenthiran' ); ?></li>
								<li><code>[latepoint_book_form]</code> — <?php esc_html_e( 'inline form (behavior varies by LatePoint version — may show customer fields first)', 'raveenthiran' ); ?></li>
								<li><code>[latepoint_book_form selected_service="123"]</code> — <?php esc_html_e( 'inline, pre-select a specific service by ID', 'raveenthiran' ); ?></li>
							</ul>
							<p class="description" style="margin-top:8px">
								<?php esc_html_e( 'If you only see the customer-details form (name/email) instead of the calendar, your LatePoint flow is probably starting at the wrong step. In LatePoint admin → Settings → Booking → "Start booking process from", set it to the calendar/date-time step.', 'raveenthiran' ); ?>
							</p>
						</td></tr>
