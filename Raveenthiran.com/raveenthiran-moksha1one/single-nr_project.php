<?php
/**
 * VOID single specimen — a technical dossier, now a HORIZONTAL filmstrip: the
 * analysis header, then the primary frame beside its HUD readouts, then every
 * gallery plate on its own panel. Vertical wheel travels sideways on desktop
 * (mk-scroll.js); natural vertical stack on touch. Data via the nr_* engine.
 */
...
$extra  = max( 0, count( $plates ) - 1 );          // plate panels after the hero
$total  = 2 + $extra;                               // dossier + hero + plates
$pnum   = 0;
$ix     = function () use ( &$pnum, $total ) { return str_pad( (string) ++$pnum, 2, '0', STR_PAD_LEFT ) . ' / ' . str_pad( (string) $total, 2, '0', STR_PAD_LEFT ); };
?>
...
<div class="mk-h" data-mk-h>
	<div class="mk-h__track">
		...
		<?php /* ── dossier ─────────────────────────────────────── */ ?>
		<section class="mk-panel mk-dossier">
			...
			<div class="void-meta-rows" style="margin-top:22px">
			...
			<?php if ( $lede ) : ?>
			<div class="void-note-box" style="margin-top:20px">
				<span class="void-label-luxury"><?php esc_html_e( 'ANALYSIS', 'raveenthiran' ); ?></span>
				<p><?php echo esc_html( $lede ); ?></p>
			</div>
			<?php endif; ?>

			<a class="void-btn void-btn-gold void-btn-wide" style="margin-top:22px;align-self:flex-start" href="<?php echo esc_url( add_query_arg( [ 'service' => sanitize_title( $m['cat'] ?? 'editorial' ), 'ref' => get_the_title() ], function_exists( 'nr_enquire_url' ) ? nr_enquire_url() : home_url( '/enquire' ) ) ); ?>"><?php echo esc_html( nr_opt( 'nr_cta_commission', __( 'COMMISSION SIMILAR', 'raveenthiran' ) ) ); ?></a>

			<div class="void-single-nav" style="margin-top:30px">
				<?php if ( $prev ) : ?>
		...
		<?php /* ── primary frame + HUD ─────────────────────────── */ ?>
		<section class="mk-panel">
			<span class="mk-panel__index"><?php echo esc_html( $ix() ); ?></span>
			<div class="mk-work mk-specimen">
				<div class="void-single-center" style="position:relative">
				...
				<div class="void-single-right mk-hud">
		...
		<?php /* ── gallery plates — one per panel ───────────────── */ ?>
		<?php foreach ( array_slice( $plates, 1 ) as $k => $pid ) : ?>
		<section class="mk-panel mk-plate">
			<span class="mk-panel__index"><?php echo esc_html( $ix() ); ?></span>
			<figure class="void-frame void-frame--plate mk-plate__frame">
