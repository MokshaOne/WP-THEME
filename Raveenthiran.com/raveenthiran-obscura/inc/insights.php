...
function nr_enquiry_insights_render() {
	...
	$export = function_exists( 'nr_enquiry_export_url' )
		...
		: '';
	if ( function_exists( 'nr_enquiry_export_enc_url' ) ) {
		$export .= sprintf( ' <a href="%s" class="button button-small">%s</a>', esc_url( nr_enquiry_export_enc_url() ), esc_html__( 'Export 🔒', 'raveenthiran' ) );
	}
	printf(
