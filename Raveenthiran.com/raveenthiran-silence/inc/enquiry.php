 ...
 * Nonce + honeypot + rate limit, saves a private sl_enquiry post,
 * mails the studio. No plugins.
 */
if ( ! defined( 'ABSPATH' ) ) exit;

add_action( 'template_redirect', function () {
	if ( $_SERVER['REQUEST_METHOD'] !== 'POST' || ! isset( $_POST['sl_enquiry_nonce'] ) ) return;
	...
	if ( ! wp_verify_nonce( $_POST['sl_enquiry_nonce'], 'sl_enquiry' ) ) $fail();
	...
	if ( function_exists( 'sl_turnstile_passes' ) && ! sl_turnstile_passes() ) $fail();
	// Rate limit — one submission per IP per minute.
	$ip_key = 'sl_enq_' . md5( $_SERVER['REMOTE_ADDR'] ?? '' );
	...
	wp_insert_post( [
		'post_type'    => 'sl_enquiry',
		'post_status'  => 'private',
		'post_title'   => $name . ' — ' . wp_date( 'Y-m-d H:i' ),
		'post_content' => "From: {$name} <{$email}>\n\n{$message}",
	] );

	$to = sl_opt( 'sl_email', get_option( 'admin_email' ) );
	wp_mail(
		$to,
