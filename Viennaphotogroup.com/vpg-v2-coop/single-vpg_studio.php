...
?>
    ...
    <header class="vpg-page-hero">
        <span class="vpg-chip vpg-chip--studio"><span class="vpg-chip__dot"></span> Studio</span>
        <h1><?php the_title(); ?></h1>
        <?php if ( get_the_excerpt() ) : ?><p class="vpg-lede"><?php echo esc_html( get_the_excerpt() ); ?></p><?php endif; ?>
        <?php if ( $meta ) : ?>
            <p class="vpg-caps" style="margin-top:1.5rem">Rate · <?php echo esc_html( $meta ); ?></p>
        <?php endif; ?>
    ...
    <section class="vpg-section vpg-section--surface vpg-section--tight" style="text-align:center">
        <div class="vpg-wrap--narrow">
            <a class="vpg-btn vpg-btn--ghost" href="<?php echo esc_url( get_post_type_archive_link( 'vpg_studio' ) ); ?>"><?php esc_html_e( 'All studios', 'vpg-v2' ); ?> →</a>
        </div>
    </section>
