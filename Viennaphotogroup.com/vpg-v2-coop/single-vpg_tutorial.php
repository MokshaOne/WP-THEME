...
?>
    ...
    <header class="vpg-page-hero">
        <span class="vpg-chip vpg-chip--tut"><span class="vpg-chip__dot"></span> Tutorial</span>
        <h1><?php the_title(); ?></h1>
        <?php if ( get_the_excerpt() ) : ?><p class="vpg-lede"><?php echo esc_html( get_the_excerpt() ); ?></p><?php endif; ?>
        <?php if ( $meta ) : ?>
            <p class="vpg-caps" style="margin-top:1.5rem">Min · <?php echo esc_html( $meta ); ?></p>
        <?php endif; ?>
    ...
    <section class="vpg-section vpg-section--surface vpg-section--tight" style="text-align:center">
        <div class="vpg-wrap--narrow">
            <a class="vpg-btn vpg-btn--ghost" href="<?php echo esc_url( get_post_type_archive_link( 'vpg_tutorial' ) ); ?>"><?php esc_html_e( 'All tutorials', 'vpg-v2' ); ?> →</a>
        </div>
    </section>
