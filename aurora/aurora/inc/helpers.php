...
function AURORA_mod( $key, $default = '' ) {
    $v = get_theme_mod( 'AURORA_' . $key, $default );
    return $v !== '' ? $v : $default;
}

function AURORA_em( $html ) {
    return wp_kses( $html, [ 'em' => [], 'strong' => [], 'br' => [], 'span' => [ 'class' => [] ], 'small' => [], 'sup' => [] ] );
}

function AURORA_get_identity() {
    return [
        'brand'    => get_bloginfo( 'name' ),
        'tagline'  => get_bloginfo( 'description' ),
        'email'    => AURORA_mod( 'email',    get_option( 'admin_email' ) ),
        'location' => AURORA_mod( 'location', 'Wien · UTC+1' ),
        'booking'  => AURORA_mod( 'booking',  'Q3 · slots open' ),
        'since'    => AURORA_mod( 'since',    '2018' ),
        'sites'    => AURORA_mod( 'sites',    '14' ),
        'reply'    => AURORA_mod( 'reply',    '72' ),
    ];
}

function AURORA_get_hero() {
    return [
        'eyebrow' => AURORA_mod( 'hero_eyebrow', 'An atelier · Wien · since MMXVIII' ),
        'line1'   => AURORA_mod( 'hero_line1',   'where craft' ),
        'line2'   => AURORA_mod( 'hero_line2',   'meets code' ),
        'lede'    => AURORA_mod( 'hero_lede',    'A one-person digital atelier crafting editorial WordPress sites and design systems for studios who want to look like themselves, not like a template.' ),
        'cta'     => AURORA_mod( 'hero_cta',     'Start a project' ),
        'cta_url' => AURORA_mod( 'hero_cta_url', '#brief' ),
    ];
}

function AURORA_get_principles() {
    $raw = AURORA_mod( 'principles', "i. | Quiet design is the most generous kind of work\nii. | One person responsible — brief to ship\niii. | Editorial over template, always\niv. | Code that lasts a decade, not a quarter" );
    $lines = preg_split( '/\r\n|\r|\n/', $raw );
    $out = [];
...
function AURORA_get_work() {
    // Prefer custom-post-type entries if any are published; otherwise fall back to Customizer text.
    $query = new WP_Query( [ 'post_type' => 'AURORA_project', 'posts_per_page' => 12, 'orderby' => 'menu_order date', 'order' => 'DESC' ] );
    if ( $query->have_posts() ) {
        $out = [];
        while ( $query->have_posts() ) {
            $query->the_post();
            $out[] = [
                'year'  => get_post_meta( get_the_ID(), 'AURORA_year',  true ) ?: get_the_date( 'Y' ),
                'title' => get_the_title(),
                'lede'  => get_the_excerpt() ?: get_post_meta( get_the_ID(), 'AURORA_lede', true ),
                'url'   => get_permalink(),
            ];
        }
        wp_reset_postdata();
