<?php
/**
 * Template Name: நூல்கள்
 *
 * Books page — assign this template to a WP page you create
 * (e.g., title 'நூல்கள்', slug 'nulgal' or anything else).
 *
 * Renders all published kavithai_nul posts grouped by decade,
 * with the v2.1 Relief books grid. Drops in the empty state
 * if no books are published yet. Identical visual to the CPT
 * archive at /nulgal/ — this is the page-template version so
 * the URL is yours to define.
 */
...
<header class="page-head">
    ...
    <div class="page-head__in">
        ...
        <h1 class="page-head__title"><?php echo esc_html( get_the_title() ?: 'நூல்கள்' ); ?></h1>
        <p class="page-head__sub">
            Published Works
            <?php if ( $total > 0 ) : ?>
                <span style="color:var(--ochre);margin:0 0.5em;">·</span> <span class="la"><?php echo $total; ?> volume<?php echo $total === 1 ? '' : 's'; ?></span>
            <?php endif; ?>
        </p>
    </div>
</header>
...
<?php
// Optional intro text from the WP page body.
$page_intro = get_the_content();
if ( trim( wp_strip_all_tags( $page_intro ) ) !== '' ) : ?>
    <section style="max-width: 720px; margin: 0 auto; padding: var(--pad-section) var(--pad-x) 0; font-family: var(--serif-ta); font-size: 1.18rem; line-height: 2; color: var(--ink-soft);">
        <?php the_content(); ?>
    </section>
<?php endif; ?>
...
<section class="books">
    <?php if ( empty( $by_decade ) ) : ?>
        <p style="font-family:var(--serif-ta); font-style:italic; color:var(--ink-mute); text-align:center; padding:4rem 0;">
            இங்கே இன்னும் நூல்கள் இல்லை.
            <br><br>
            <span style="font-family:var(--serif-la); font-size:0.85rem; letter-spacing:0.15em; text-transform:uppercase; color:var(--ink-faint);">
                Add a new book via <em>நூல்கள் → புதிய நூல்</em>
            </span>
        </p>
