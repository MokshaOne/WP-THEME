...
- *Optional:* set a **booking URL** (Cal.com, etc.) to embed in the booking modal; otherwise the native inline brief form is used
...
├── parts/
│   ├── booking-modal.php      Booking modal (iframe host or inline brief form)
...
- **Booking modal** — opens via any link with `data-modal="nr-booking"`. Embeds your booking URL in an iframe if set; otherwise renders the inline brief form.
