...
function ORBIT_mod( $key, $default = '' ) {
    $v = get_theme_mod( 'ORBIT_' . $key, $default );
    return $v !== '' ? $v : $default;
}

function ORBIT_em( $html ) {
    return wp_kses( $html, [ 'em' => [], 'strong' => [], 'br' => [], 'span' => [ 'class' => [] ], 'small' => [], 'sup' => [] ] );
}

function ORBIT_get_identity() {
    return [
        'brand'    => get_bloginfo( 'name' ),
        'tagline'  => get_bloginfo( 'description' ),
        'email'    => ORBIT_mod( 'email',    get_option( 'admin_email' ) ),
        'location' => ORBIT_mod( 'location', 'Wien · UTC+1' ),
        'booking'  => ORBIT_mod( 'booking',  'Q3 · slots open' ),
        'since'    => ORBIT_mod( 'since',    '2018' ),
        'sites'    => ORBIT_mod( 'sites',    '14' ),
        'reply'    => ORBIT_mod( 'reply',    '72' ),
    ];
}

function ORBIT_get_hero() {
    return [
        'eyebrow' => ORBIT_mod( 'hero_eyebrow', 'An atelier · Wien · since MMXVIII' ),
        'line1'   => ORBIT_mod( 'hero_line1',   'where craft' ),
        'line2'   => ORBIT_mod( 'hero_line2',   'meets code' ),
        'lede'    => ORBIT_mod( 'hero_lede',    'A one-person digital atelier crafting editorial WordPress sites and design systems for studios who want to look like themselves, not like a template.' ),
        'cta'     => ORBIT_mod( 'hero_cta',     'Start a project' ),
        'cta_url' => ORBIT_mod( 'hero_cta_url', '#brief' ),
    ];
}

function ORBIT_get_principles() {
    $raw = ORBIT_mod( 'principles', "i. | Quiet design is the most generous kind of work\nii. | One person responsible — brief to ship\niii. | Editorial over template, always\niv. | Code that lasts a decade, not a quarter" );
    $lines = preg_split( '/\r\n|\r|\n/', $raw );
    $out = [];
...
function ORBIT_get_work() {
    // Prefer custom-post-type entries if any are published; otherwise fall back to Customizer text.
    $query = new WP_Query( [ 'post_type' => 'ORBIT_project', 'posts_per_page' => 12, 'orderby' => 'menu_order date', 'order' => 'DESC' ] );
    if ( $query->have_posts() ) {
        $out = [];
        while ( $query->have_posts() ) {
            $query->the_post();
            $out[] = [
                'year'  => get_post_meta( get_the_ID(), 'ORBIT_year',  true ) ?: get_the_date( 'Y' ),
                'title' => get_the_title(),
                'lede'  => get_the_excerpt() ?: get_post_meta( get_the_ID(), 'ORBIT_lede', true ),
                'url'   => get_permalink(),
            ];
        }
        wp_reset_postdata();
