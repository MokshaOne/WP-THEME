...
function REVOLT_mod( $key, $default = '' ) {
    $v = get_theme_mod( 'REVOLT_' . $key, $default );
    return $v !== '' ? $v : $default;
}

function REVOLT_em( $html ) {
    return wp_kses( $html, [ 'em' => [], 'strong' => [], 'br' => [], 'span' => [ 'class' => [] ], 'small' => [], 'sup' => [] ] );
}

function REVOLT_get_identity() {
    return [
        'brand'    => get_bloginfo( 'name' ),
        'tagline'  => get_bloginfo( 'description' ),
        'email'    => REVOLT_mod( 'email',    get_option( 'admin_email' ) ),
        'location' => REVOLT_mod( 'location', 'Wien · UTC+1' ),
        'booking'  => REVOLT_mod( 'booking',  'Q3 · slots open' ),
        'since'    => REVOLT_mod( 'since',    '2018' ),
        'sites'    => REVOLT_mod( 'sites',    '14' ),
        'reply'    => REVOLT_mod( 'reply',    '72' ),
    ];
}

function REVOLT_get_hero() {
    return [
        'eyebrow' => REVOLT_mod( 'hero_eyebrow', 'An atelier · Wien · since MMXVIII' ),
        'line1'   => REVOLT_mod( 'hero_line1',   'where craft' ),
        'line2'   => REVOLT_mod( 'hero_line2',   'meets code' ),
        'lede'    => REVOLT_mod( 'hero_lede',    'A one-person digital atelier crafting editorial WordPress sites and design systems for studios who want to look like themselves, not like a template.' ),
        'cta'     => REVOLT_mod( 'hero_cta',     'Start a project' ),
        'cta_url' => REVOLT_mod( 'hero_cta_url', '#brief' ),
    ];
}

function REVOLT_get_principles() {
    $raw = REVOLT_mod( 'principles', "i. | Quiet design is the most generous kind of work\nii. | One person responsible — brief to ship\niii. | Editorial over template, always\niv. | Code that lasts a decade, not a quarter" );
    $lines = preg_split( '/\r\n|\r|\n/', $raw );
    $out = [];
...
function REVOLT_get_work() {
    // Prefer custom-post-type entries if any are published; otherwise fall back to Customizer text.
    $query = new WP_Query( [ 'post_type' => 'REVOLT_project', 'posts_per_page' => 12, 'orderby' => 'menu_order date', 'order' => 'DESC' ] );
    if ( $query->have_posts() ) {
        $out = [];
        while ( $query->have_posts() ) {
            $query->the_post();
            $out[] = [
                'year'  => get_post_meta( get_the_ID(), 'REVOLT_year',  true ) ?: get_the_date( 'Y' ),
                'title' => get_the_title(),
                'lede'  => get_the_excerpt() ?: get_post_meta( get_the_ID(), 'REVOLT_lede', true ),
                'url'   => get_permalink(),
            ];
        }
        wp_reset_postdata();
