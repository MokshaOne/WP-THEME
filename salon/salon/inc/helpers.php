...
function SALON_mod( $key, $default = '' ) {
    $v = get_theme_mod( 'SALON_' . $key, $default );
    return $v !== '' ? $v : $default;
}

function SALON_em( $html ) {
    return wp_kses( $html, [ 'em' => [], 'strong' => [], 'br' => [], 'span' => [ 'class' => [] ], 'small' => [], 'sup' => [] ] );
}

function SALON_get_identity() {
    return [
        'brand'    => get_bloginfo( 'name' ),
        'tagline'  => get_bloginfo( 'description' ),
        'email'    => SALON_mod( 'email',    get_option( 'admin_email' ) ),
        'location' => SALON_mod( 'location', 'Wien · UTC+1' ),
        'booking'  => SALON_mod( 'booking',  'Q3 · slots open' ),
        'since'    => SALON_mod( 'since',    '2018' ),
        'sites'    => SALON_mod( 'sites',    '14' ),
        'reply'    => SALON_mod( 'reply',    '72' ),
    ];
}

function SALON_get_hero() {
    return [
        'eyebrow' => SALON_mod( 'hero_eyebrow', 'An atelier · Wien · since MMXVIII' ),
        'line1'   => SALON_mod( 'hero_line1',   'where craft' ),
        'line2'   => SALON_mod( 'hero_line2',   'meets code' ),
        'lede'    => SALON_mod( 'hero_lede',    'A one-person digital atelier crafting editorial WordPress sites and design systems for studios who want to look like themselves, not like a template.' ),
        'cta'     => SALON_mod( 'hero_cta',     'Start a project' ),
        'cta_url' => SALON_mod( 'hero_cta_url', '#brief' ),
    ];
}

function SALON_get_principles() {
    $raw = SALON_mod( 'principles', "i. | Quiet design is the most generous kind of work\nii. | One person responsible — brief to ship\niii. | Editorial over template, always\niv. | Code that lasts a decade, not a quarter" );
    $lines = preg_split( '/\r\n|\r|\n/', $raw );
    $out = [];
...
function SALON_get_work() {
    // Prefer custom-post-type entries if any are published; otherwise fall back to Customizer text.
    $query = new WP_Query( [ 'post_type' => 'SALON_project', 'posts_per_page' => 12, 'orderby' => 'menu_order date', 'order' => 'DESC' ] );
    if ( $query->have_posts() ) {
        $out = [];
        while ( $query->have_posts() ) {
            $query->the_post();
            $out[] = [
                'year'  => get_post_meta( get_the_ID(), 'SALON_year',  true ) ?: get_the_date( 'Y' ),
                'title' => get_the_title(),
                'lede'  => get_the_excerpt() ?: get_post_meta( get_the_ID(), 'SALON_lede', true ),
                'url'   => get_permalink(),
            ];
        }
        wp_reset_postdata();
